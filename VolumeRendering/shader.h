...
public:
	...
	void setColor(const std::string &name, nanogui::Color color) {
		glUniform3fv(glGetUniformLocation(program, name.c_str()), 1, glm::value_ptr(glm::vec3(color.r(), color.g(), color.b())));
	}
	void setMatrix4(const std::string &name, const glm::mat4 &mat) const
	{
		glUniformMatrix4fv(glGetUniformLocation(program, name.c_str()), 1, GL_FALSE, &mat[0][0]);
	}
	void setValue(const std::string &name, const float value) const
	{
